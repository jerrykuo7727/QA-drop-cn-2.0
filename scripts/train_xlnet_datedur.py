...
from prepare_xlnet_drop_data import date_duration_op
    ...
    for bi, batch in enumerate(dataloader, start=1):
        ...
        for i, answer in enumerate(answers):
            ...
            for k in range(model.start_n_top):
                op_type = op_types[i]
                assert op_type in (0,1,2)
                start_ind = start_index[i][k]
                end_ind = end_index[i][k]
                input_tokens = input_tokens_no_unk[i]
                pred = date_duration_op(tokenizer, date_dur_par_re, date_dur_full_re, date_re, dur_re, tn, \
...
def validate(model, tokenizer, topk=1):
    ...
    test_avg_em = 0.0  #100 * test_em / test_count
    test_avg_f1 = 0.0  #100 * test_f1 / test_count
...
if __name__ == '__main__':
    ...
    #ckpt = torch.load('/home/M10815022/disk1/Workshop/QA-FGC-xlnet/models/xlnet/finetune.ckpt', 'cpu')
    #model.load_state_dict(ckpt)
    ckpt = torch.load('models/sinspan_xlnet_only.ckpt', 'cpu')
